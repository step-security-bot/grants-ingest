...
type Environment struct {
	...
	SourceDataBucket  string `env:"GRANTS_SOURCE_DATA_BUCKET_NAME,required=true"`
	ValidFFISEmail    string `env:"FFIS_DIGEST_EMAIL_ADDRESS,required=true"`
...
var (
	env        Environment
	logger     log.Logger
	sendMetric = ddHelpers.NewMetricSender("PrepareFFISEmail", "source:grants.gov")
)
...
func main() {
	...
	lambda.Start(ddlambda.WrapFunction(func(ctx context.Context, s3Event events.S3Event) error {
		...
		log.Debug(logger, "Starting Lambda")
		return handleS3EventWithConfig(s3Svc, ctx, s3Event)
	}, nil))
}
